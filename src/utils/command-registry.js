 ...
 * @type {Array<{name: string, hint: string, run: Function, when: Function|null}>}
 */
const commands = [];
 ...
 *   arguments; the palette closes first and catches whatever this throws
 ...
 *   a target that has to be on the page. Omitted means always.
 * @returns {boolean} Whether it was accepted
 */
export function registerCommand({ name, hint = '', run, when = null }) {
...
export function registeredCommands() {
    return commands
        ...
        .map(({ command }) => ({ name: command.name, hint: command.hint, run: command.run }));
}
