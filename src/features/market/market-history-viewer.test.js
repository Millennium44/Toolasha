...
describe('the "When it trades" item list', () => {
    beforeEach(() => {
        marketHistoryViewer.itemNameCache.clear();
        const names = { '/items/cheese': 'Cheese', '/items/apple': 'Apple' };
        dataManagerMock.getItemDetails = (hrid) => (names[hrid] ? { name: names[hrid] } : null);
        marketHistoryViewer.filters = {
            dateFrom: null,
            dateTo: null,
            selectedItems: [],
    ...
    test('each item and level once, by name', () => {
        marketHistoryViewer.listings = [
            { id: 1, timestamp: 1000, itemHrid: '/items/cheese', enhancementLevel: 0 },
            { id: 2, timestamp: 3000, itemHrid: '/items/cheese', enhancementLevel: 0 },
            { id: 3, timestamp: 2000, itemHrid: '/items/apple', enhancementLevel: 0 },
            { id: 4, timestamp: 1500, itemHrid: '/items/cheese', enhancementLevel: 5 },
        ];
        expect(marketHistoryViewer.clockItems().map((i) => [i.itemHrid, i.enhancementLevel])).toEqual([
            ['/items/apple', 0],
            ['/items/cheese', 0],
            ['/items/cheese', 5],
        ]);
    ...
    test('opens on the filtered item, else the most recently listed one', () => {
        marketHistoryViewer.listings = [
            { id: 1, timestamp: 1000, itemHrid: '/items/cheese', enhancementLevel: 0 },
            { id: 3, timestamp: 2000, itemHrid: '/items/apple', enhancementLevel: 0 },
        ];
        expect(marketHistoryViewer.clockInitialItem()).toEqual({ itemHrid: '/items/apple', enhancementLevel: 0 });

        marketHistoryViewer.filters.selectedItems = ['/items/cheese'];
        expect(marketHistoryViewer.clockInitialItem()).toEqual({ itemHrid: '/items/cheese', enhancementLevel: 0 });
    });
});
