...
- **Session log you can page through, plus export and clear.** Every clicked monster's comparison is kept for the session, deduped by monster and room level and capped. A nav bar pages back through them — ▲/▼ buttons or, while the pointer is over the panel, the ↑/↓ arrows — with a position and "how long ago" readout; a fresh click jumps back to live. A **Clear** button wipes the log for a fresh survey, and the ⭳ button downloads the log plus the current snapshot as JSON. `Toolasha.Debug.monsterStatCheck()` dumps the last comparison as a table; `Toolasha.Debug.monsterStatCheckLog()` returns the whole log.
