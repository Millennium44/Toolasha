...
- **A chooser Toolasha just opened is given a beat to become interactive before an option is pressed.** Pressing a reroll option in the first frame after the menu opens reached no server. The decision of which option to take still runs on what the opener settled on, so a pass already seen to be spent is not waited for again; only the press itself is deferred. Two temporary diagnostic lines (`pressing reroll option: …` and `reroll confirmed / NOT confirmed`) are logged while the DOM path is being replaced (below).
- **Groundwork for driving the reroll off the WebSocket instead of the DOM.** The diagnostics proved the remaining failure is not a Toolasha bug: the game gates a reroll on a _trusted_ click that opens its chooser, and a userscript's synthetic open leaves the reroll inert (a hand-opened menu rerolls; a Toolasha-opened one does not). So the socket hook now captures the live game socket, exposes a `send()` for replaying the game's own reroll message, and — temporarily — logs outgoing frames (`[Toolasha WS-OUT]`) so that exact message can be learned. The DOM press stays in place meanwhile, so a reroll on an already-open menu keeps working.
