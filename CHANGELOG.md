...
- **A chooser Toolasha just opened is given a beat to become interactive before an option is pressed.** Pressing a reroll option in the first frame after the menu opens reached no server — the reroll only landed when the menu had already been open by hand (and so had long since settled). The decision of which option to take still runs on what the opener settled on, so a pass already seen to be spent is not waited for again; only the press itself is deferred. Two temporary diagnostic lines (`pressing reroll option: …` and `reroll confirmed / NOT confirmed`) are logged so a reroll that still fails to land can be pinpointed.

### The combat/enhancement workers stop logging a math.js source-map error

Two worker pools (enhancement calculations and networth valuation) load math.js from cdnjs with `importScripts`, and that build ends with a source-map directive pointing at math.js.map. The map is not served in this context, so every worker spawn logged a "Source map error … in the worker" (`JSON.parse: unexpected character`). The workers now fetch the math.js source, drop that one trailing directive, and run the rest from a same-origin blob — no map is requested, so the error is gone. If the fetch/blob path fails for any reason it falls back to importing the CDN URL directly, so the maths is never at risk; only the console noise is removed. The loader is shared between both worker managers so they cannot drift on it.
