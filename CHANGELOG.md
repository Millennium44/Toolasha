...
- The **Remove** cell stays put. The reorder used to lift every tile out and put them all back at one anchor, which displaced anything else sharing the grid; it now permutes the tiles among the slots they already occupy, so nothing that is not a tile is touched at all. A cell standing for no item is also kept at the front of the ordering, for the case where it is one of the grid's tiles.
